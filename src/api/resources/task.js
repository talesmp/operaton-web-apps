import { RESPONSE_STATE, _url, GET, GET_FORM, GET_TEXT, POST, PUT } from '../helper.jsx'
...
const update_task = (state, task, task_id) => {
  ...
  console.log({ ...state.api.task.one.value.data, ...task })

  return state.api.task.one.value?.data !== undefined
    ? PUT(`/task/${state.api.task.one.value.data.id}`, { ...state.api.task.one.value.data, ...task }, state, state.api.task.one)
...
const add_group = (state, task_id, groupId) =>
  POST(`/task/${task_id}/identity-links`, { groupId, type: 'candidate' }, state, state.api.task.add_group)

const delete_group = (state, task_id, groupId) =>
  POST(`/task/${task_id}/identity-links/delete`, { groupId, type: 'candidate' }, state, state.api.task.delete_group)
